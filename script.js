...
function initDropdownMenu() {
    const dropdownTrigger = document.querySelector('.nav__item-sp');
    const dropdownMenu = document.querySelector('.dropdown-menu');
    
    if (!dropdownTrigger || !dropdownMenu) return;
    
    // Открытие/закрытие при наведении
    dropdownTrigger.addEventListener('mouseenter', openDropdown);
    dropdownTrigger.parentElement.addEventListener('mouseleave', closeDropdown);
    ...
    if (window.innerWidth <= 768) {
        dropdownTrigger.addEventListener('click', handleMobileDropdownClick);
    }
...
// ==================== ВЫПАДАЮЩИЕ МЕНЮ С СТРЕЛКАМИ ====================
function initDropdownMenusWithArrows() {
        ...
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (element.classList.contains('service-card__img')) {
                    const index = parseInt(element.getAttribute('data-index') || '0');
                    setTimeout(() => animateServiceCardImage(element, index), index * 300);
                }
